#!/usr/bin/env bash
# Bump the bmcctl-bin AUR package to the version of the latest GitHub release.
#
...
#       https://github.com/j4y-w4lk3r/bmcctl/releases/tag/v<version>
#     with bmcctl_<version>_Linux_x86_64.tar.gz attached (goreleaser does this).
#
# What it does:
#   1. Clones (or refreshes) the AUR bmcctl-bin repo into /tmp/bmcctl-bin-aur
...
if command -v makepkg >/dev/null 2>&1; then
    ...
    sed -i.bak "s|/v[0-9.]*/bmcctl_[0-9.]*_Linux_x86_64\.tar\.gz|/v${VER}/bmcctl_${VER}_Linux_x86_64.tar.gz|" .SRCINFO
    rm -f .SRCINFO.bak
fi
...
if git diff --cached --quiet; then
    ...
    git commit -m "bmcctl-bin: bump to ${VER}"
    git push origin HEAD:master
fi

echo
echo "[OK] bmcctl-bin bumped to v${VER} on AUR."
