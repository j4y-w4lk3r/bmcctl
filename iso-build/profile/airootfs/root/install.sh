...
# Fetch a single-use Headscale preauth key from the build host (install-host.sh
# serves it once over HTTP). The key is never baked into the ISO — only the
# fetch URL (LAN IP + port) is in install-config.env.
_fetch_headscale_authkey() {
    local host="${HEADSCALE_KEY_FETCH_HOST:-}" port="${HEADSCALE_KEY_FETCH_PORT:-9765}" key=""
    [[ -n $host ]] || return 1
    echo "::: fetching single-use Headscale key from http://${host}:${port}/headscale-key" >&2
    for _ in $(seq 1 90); do
        key=$(curl -fsS --max-time 3 "http://${host}:${port}/headscale-key" 2>/dev/null) || true
        if [[ -n $key && $key == hskey-auth-* ]]; then
            echo "::: headscale preauth key received" >&2
            printf '%s' "$key"
            return 0
        fi
        sleep 2
...
arch-chroot /mnt /usr/bin/env \
    ...
    HEADSCALE_ENABLED="${HEADSCALE_ENABLED:-false}" \
...
# Users — zsh only (no bash login shell or skel dotfiles).
...
# ---- first-boot Headscale join (fetch key live, write only to disk) ----
if [[ "${HEADSCALE_ENABLED:-false}" =~ ^([Tt][Rr][Uu][Ee]|[Yy][Ee][Ss]|1)$ ]]; then
    echo "::: enabling first-boot Headscale join (hostname=${HEADSCALE_HOSTNAME:-$HOSTNAME})"
    HS_AUTHKEY=""
    if ! HS_AUTHKEY=$(_fetch_headscale_authkey); then
        echo "WARN: could not fetch Headscale preauth key from build host — skipping join"
    elif [[ ! -f /usr/local/lib/bmcctl/headscale-join.sh || ! -f /etc/systemd/system/bmcctl-headscale.service ]]; then
        echo "WARN: headscale assets missing from live ISO — skipping; join manually after boot"
    else
        {
            mkdir -p /mnt/etc/bmcctl /mnt/usr/local/lib/bmcctl /mnt/etc/systemd/system &&
            install -m600 /dev/null /mnt/etc/bmcctl/headscale.env &&
            cat > /mnt/etc/bmcctl/headscale.env <<HSENV
HEADSCALE_LOGIN_SERVER=${HEADSCALE_LOGIN_SERVER:-https://hs.d0j0.dev}
HEADSCALE_HOSTNAME=${HEADSCALE_HOSTNAME:-$HOSTNAME}
HEADSCALE_ACCEPT_ROUTES=${HEADSCALE_ACCEPT_ROUTES:-true}
HEADSCALE_ACCEPT_DNS=${HEADSCALE_ACCEPT_DNS:-true}
HEADSCALE_AUTHKEY=${HS_AUTHKEY}
HSENV
            chmod 600 /mnt/etc/bmcctl/headscale.env &&
            install -m755 /usr/local/lib/bmcctl/headscale-join.sh /mnt/usr/local/lib/bmcctl/ &&
            install -m644 /etc/systemd/system/bmcctl-headscale.service /mnt/etc/systemd/system/ &&
            arch-chroot /mnt systemctl enable bmcctl-headscale.service
        } || echo "WARN: first-boot headscale enablement failed (non-fatal) — run tailscale up manually after boot"
    fi
fi
...
INSTALLED_DATE=$(date -u -d "${INSTALLED_AT}" +%d/%m/%Y 2>/dev/null || date -u +%d/%m/%Y)
INSTALLED_TIME=$(date -u -d "${INSTALLED_AT}" +%H:%M 2>/dev/null || date -u +%H:%M)
ISO_LABEL=$(cat /run/archiso/bootmnt/arch/version 2>/dev/null \
    || cat /etc/arch-release 2>/dev/null || echo "bmcctl-iso")
mkdir -p /mnt/etc/motd.d /mnt/usr/local/lib/bmcctl
...
# Colored MOTD (pam_motd on SSH login) + same facts for zsh greeting module.
# Date: DD/MM/YYYY  time: HH:MM (UTC)
C=$'\033[38;5;81m' T=$'\033[38;5;120m' L=$'\033[38;5;245m' V=$'\033[1;37m' R=$'\033[0m'
{
  printf '%b\n' \
    "${C}  ┌─ ${T}bmcctl fresh install${C} ─────────────────────────────────────${R}" \
    "${C}  │${R}  ${L}host:${R}        ${V}${HOSTNAME}${R}" \
    "${C}  │${R}  ${L}installed:${R}   ${V}${INSTALLED_DATE} ${INSTALLED_TIME} (UTC)${R}" \
    "${C}  │${R}  ${L}install-id:${R}  ${V}${INSTALL_ID}${R}" \
    "${C}  └────────────────────────────────────────────────────────────${R}"
} > /mnt/etc/motd.d/10-bmcctl-install.conf
chmod 644 /mnt/etc/motd.d/10-bmcctl-install.conf
if [[ -f /usr/local/lib/bmcctl/homelab-registry-push.sh ]]; then
    install -m755 /usr/local/lib/bmcctl/homelab-registry-push.sh \
        /mnt/usr/local/lib/bmcctl/homelab-registry-push.sh
fi
